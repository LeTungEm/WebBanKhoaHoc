<?php
    ...
    if(isset($_POST["btnSubmitLg"])){
        ...
        //Khách hàng hoặc nhân viên đăng nhập
        ...
        if($data != null && $data[0]["passWord"] == $mk){
            ...
            //Tài khoản là của nhân viên
            $data = $employee->getAccount($mk, $sdt);
            if($data != null && $data[0]["passWord"] == $mk){
                //Có dữ liệu nhân viên
                $_SESSION["employee_sdt"] = $sdt;
                if($ckbLuu == 1){
                    setcookie("employee_sdt", $sdt, time() + (86400 * 30), "/");
                }
                header("location:index.php?action=home");
            }else{
